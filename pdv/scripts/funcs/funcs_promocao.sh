...
criar_promocao() {
  log "Iniciando criação de promoção."
  ...
  [ $? -ne 0 ] && log "Criação de promoção cancelada pelo usuário." && return
  ...
  if [ "$PRODUTO_EXISTENTE" -eq 0 ]; then
    ...
    log "Produto não encontrado para o código $CODIGO."
    return 1
  fi
  ...
  log "Produto $NOME com código $CODIGO encontrado. Preço atual: $PRECO_ATUAL."
  ...
  [ $? -ne 0 ] && log "Criação de promoção cancelada após exibição de informações do produto." && return
  ...
  [ $? -ne 0 ] && log "Criação de promoção cancelada na entrada do preço promocional." && return
  ...
  if [ $(echo "$PRECO_PROMOCAO >= $PRECO_ATUAL" | bc) -eq 1 ]; then
    ...
    log "Erro ao criar promoção: preço promocional ($PRECO_PROMOCAO) é maior ou igual ao preço atual ($PRECO_ATUAL)."
    return 1
  fi
  ...
  [ $? -ne 0 ] && log "Criação de promoção cancelada na seleção de data." && return
  ...
  log "Promoção criada com sucesso para o produto $NOME com preço promocional de $PRECO_PROMOCAO até $DATA_FINAL_PROMOCAO."
...
consultar_promocao() {
  log "Consultando promoção para um produto."

  CODIGO=$(dialog --stdout --inputbox "Digite o código GTIN ou Interno do produto:" 0 0)
  [ $? -ne 0 ] && log "Consulta de promoção cancelada pelo usuário." && return
  ...
  if [ "$PROMOCAO_EXISTENTE" -eq 0 ]; then
    ...
    log "Nenhuma promoção ativa encontrada para o produto com código $CODIGO."
    return 1
  fi
  ...
  log "Promoção ativa consultada para o produto $NOME com preço promocional $PRECO_PROMOCAO até $DATA_EXPIRACAO_FORMATADA."
}
